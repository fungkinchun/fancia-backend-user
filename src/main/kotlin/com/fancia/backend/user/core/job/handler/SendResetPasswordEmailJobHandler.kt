...
import com.fancia.backend.shared.user.core.exception.InvalidPasswordResetTokenException
...
class SendResetPasswordEmailJobHandler(
    ...
    fun run(id: UUID) {
        ...
        if (!resetToken.emailSent) {
            val user = resetToken.user ?: throw InvalidPasswordResetTokenException()
    ...
    private fun sendResetPasswordEmail(user: User, token: PasswordResetToken) {
        user.email?.let {
            ...
            emailService.sendHtmlMessage(listOf(it), "Password reset requested", htmlBody)
            token.onEmailSent()
